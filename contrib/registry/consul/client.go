...
func (c *Client) Register(ctx context.Context, svc *registry.ServiceInstance, enableHealthCheck bool) error {
	...
	if c.heartbeat {
		go func() {
			...
			if err != nil {
				log.CtxErrorf(ctx, "[Consul]update ttl heartbeat to consul failed!err:=%v", err)
			}
				...
				case <-ticker.C:
					...
					if err != nil {
						...
						if err := c.cli.Agent().ServiceRegisterOpts(asr, api.ServiceRegisterOpts{}.WithContext(cc.ctx)); err != nil {
							log.CtxErrorf(ctx, "[Consul] re registry service failed!, err=%v", err)
						} else {
							log.CtxWarn(ctx, "[Consul] re registry of service occurred success")
						}
					}
				}
			}
